...
$tempDelay = 200  # Temporisation en millisecondes
$zoomLevel = 9  # Niveau de zoom défini en dur
    ...
    if ($files.Count -gt 2) {
        ...
        if ($missingNumbers) {
            $missingNumbers | ForEach-Object {
                ...
                Log-Message -message "Missing file: $missingFileName. Attempting to download" -color "Yellow"

                # Télécharger le fichier manquant
                Download-File -destination $missingFilePath -x $x -y $y -z $z

                # Temporisation de 200ms
